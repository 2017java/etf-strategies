...
# ── 端口清理工具（恢复能力：杀掉本机端口占用者） ──
function Stop-PortListener {
    param([int]$Port)
    try {
        # 优先用 Get-NetTCPConnection（PS5+）
            ...
            foreach ($procId in $pids) {
                if ($procId -and $procId -ne 0) {
                    try {
                        ...
                        Write-ColorLine "  已清理端口 $Port 旧进程 PID=$procId" "Gray"
                    } catch {}
                }
            }
            return
...
function Stop-ProcessTree {
    ...
    # taskkill /T /F 一次性杀整棵进程树（含 cmd.exe→npm→node、python→uvicorn worker 等）
    try {
...
# 启动前清理：杀掉占用 8000 端口的旧进程（之前的能力，必须保留）
Write-ColorLine "  清理端口 8000 旧进程..." "Gray"
...
# 启动前清理：杀掉占用 5173 端口的旧进程（之前的能力，必须保留）
Write-ColorLine "  清理端口 5173 旧进程..." "Gray"
